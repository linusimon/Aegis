"""MCP Client Manager for AI Capacity Planning Advisor.

Connects to standalone mcp_db_server.py via SSE Transport (http://127.0.0.1:8001/sse)
with dynamic fallback to stdio subprocess transport.
"""
import os
import sys
import json
...
class MCPDatabaseClient:
    """Manager for communicating with the MCP SQLite Database Server over SSE or Stdio."""

    def __init__(self, sse_url: Optional[str] = None, server_script_path: Optional[str] = None, python_executable: Optional[str] = None):
        self.sse_url = sse_url or os.environ.get("MCP_SERVER_URL", "http://127.0.0.1:8001/sse")
        ...
        self.server_params = StdioServerParameters(
            ...
            args=[self.server_script_path, "--transport", "stdio"],
            env={**os.environ, "PYTHONUNBUFFERED": "1", "PYTHONPATH": os.path.dirname(os.path.dirname(os.path.abspath(__file__)))}
        )

    @asynccontextmanager
    async def _get_session(self):
        """Context manager attempting SSE connection first, falling back to stdio."""
        if sse_client and self.sse_url:
            try:
                ...
                print(f"[MCP CLIENT WARNING] SSE connection to {self.sse_url} failed ({e}). Falling back to stdio transport...")
    ...
    async def initialize_db_via_mcp(self) -> Dict[str, Any]:
        """Convenience method to run init_db tool over active transport."""
        return await self.call_tool("init_db", {})

    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Dynamically execute an MCP tool via active transport."""
